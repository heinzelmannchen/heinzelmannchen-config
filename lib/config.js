...
pub.unset = function(object, key) {
    ...
    return unsetValue(object, key);
};

function unsetValue(obj, keys) {
    var rest = _.rest(keys),
        first = _.first(keys);
    if (rest.length > 0) {
        if (!_.has(obj, first)) {
...
pub.getLocalConfigPath = function() {
    return findup.sync(SEPARATOR, HEINZEL_RC);
};

pub.getGlobalConfigPath = function() {
    return path.join('~', HEINZEL_RC);
};

pub.saveLocal = function(key, value) {
    var pathToLocalConfig = path.join(pub.getLocalConfigPath(), HEINZEL_RC);
...
pub.removeLocal = function(key) {
    ...
    return q.nfcall(fs.readFile, pathToLocalConfig, {
        ...
        .then(function(localConfig) {
            localConfig = JSON.parse(localConfig) ||  {};
            localConfig = pub.unset(localConfig, key);
            return q.nfcall(fs.writeFile, pathToLocalConfig, jsonpretty(localConfig));
        });
};

pub.saveGlobal = function(key, value) {
    var pathToGlobalConfig = path.join(pub.getGlobalConfigPath(), HEINZEL_RC);
...
pub.removeGlobal = function(key) {
    ...
    return q.nfcall(fs.readFile, pathToGlobalConfig, {
        ...
        .then(function(globalConfig) {
            globalConfig = JSON.parse(globalConfig);
            globalConfig = pub.unset(globalConfig, key);
            return q.nfcall(fs.writeFile, pathToGlobalConfig, jsonpretty(globalConfig));
        });
};
